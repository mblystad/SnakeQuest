...
class Game:
    ...
    def _init_audio(self):
        """Load background music if bg.mp3 exists, otherwise stay silent."""
        try:
        ...
        music_path = Path(__file__).parent / "bg.mp3"
    ...
    def draw_hud(self):
        ...
        score_shadow = DEFAULT_FONT.render(f"Score: {self.points}", True, shadow_color)
        level_shadow = DEFAULT_FONT.render(f"Level: {self.level}", True, shadow_color)
        time_shadow = DEFAULT_FONT.render(f"Time: {self.format_elapsed_time()}", True, shadow_color)
    ...
    def draw_start_screen(self):
        ...
        title_text = DEFAULT_FONT.render("Choose Speed", True, COLOR_HUD)
        title_rect = title_text.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT - 160))
        self.screen.blit(title_text, title_rect)

        base_y = SCREEN_HEIGHT - 120
        line_gap = 28
        for idx, (label, _) in enumerate(self.speed_options):
            color = COLOR_SNAKE if idx == self.speed_index else COLOR_HUD
            option_text = DEFAULT_FONT.render(label, True, color)
        ...
        prompt_rect = prompt_text.get_rect(midbottom=(SCREEN_WIDTH // 2, SCREEN_HEIGHT - 24))
    ...
    def draw_game_over(self):
        ...
        title_text = DEFAULT_FONT.render("Game Over", True, COLOR_HUD)
        prompt_text = DEFAULT_FONT.render(
            "Press SPACE for speed menu or ESC to exit",
            True,
            COLOR_HUD,
        )
